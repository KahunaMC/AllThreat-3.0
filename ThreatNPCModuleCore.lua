...
function ThreatLibNPCModuleCore.modulePrototype:GetTranslation(string)
	...
	if not data then
		if ThreatLib.WarnMissingTranslations then
			error("AllThreat-3.0: No translations registered for " .. selfName)
		end
		return
	end
	...
	if not data_MY_LOCALE then
		if ThreatLib.WarnMissingTranslations then
			error(("AllThreat-3.0: Missing %s translation for %q in module %q"):format(MY_LOCALE, string, selfName))
		end
